...
# :(
sim.w_pml = [0.0, w_pml, w_pml, w_pml]
sim.bc = 'E0'
...
h_wg = 0.5
waveguide = emopt.grid.Rectangle(0, Y/2, 2*X, h_wg)
waveguide.layer = 1
...
src_domain = emopt.misc.DomainCoordinates(0, 0, Y/2, Y/2, 0, 0, dx, dy, 1.0)
...
if(NOT_PARALLEL):
    ...
    f.colorbar(im)
    plt.savefig('simple_waveguide_fdtd_x_sym.pdf')
