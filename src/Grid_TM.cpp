...
double GridCell::get_intersect_angle(const Polygon_2D poly)
{
    std::vector<int> inter_ind1;
    std::vector<int> inter_ind2;
    ...
    for(int i=0; i!=out_outer.size(); ++i) {
        ...
        for(int j=0; j!=vert_outer.size(); ++j) {
            ...
            if(j==vert_outer.size()-1){
            // search if nearest neighbor point
               if(i!=0) {
                   xm1 = boost::geometry::get<0>(out_outer[i-1]);
                   ym1 = boost::geometry::get<1>(out_outer[i-1]);
               }
               else {
                   xm1 = boost::geometry::get<0>(out_outer.back());
                   ym1 = boost::geometry::get<1>(out_outer.back());
               }
               if(i!=out_outer.size()-1) {
                   xp1 = boost::geometry::get<0>(out_outer[i+1]);
                   yp1 = boost::geometry::get<1>(out_outer[i+1]);
               }
               else {
                   xp1 = boost::geometry::get<0>(out_outer[0]);
    ...
    double angle = std::atan2(-(yy1-yy0),(xx1-xx0));
    return angle;
}
...
double Rectangle::get_intersect_angle(GridCell& cell)
{
    return cell.get_intersect_angle(_poly_rep);
}
...
double Polygon::get_intersect_angle(GridCell& cell)
{
    return cell.get_intersect_angle(_verts);
}
...
std::vector<std::complex<double>> StructuredMaterial2D::get_value(double x, double y)
{
	...
	while(it != _primitives.end()) {
		        ...
		        contains_p3 || contains_p4) 
		{
			...
			if(!flag1) {
                            phi = prim->get_intersect_angle(cell_copy);
                            flag1=true;
                        }
		}
		it++;
