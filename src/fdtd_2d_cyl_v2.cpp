...
void fdtd_2d::FDTD_TE::update_H(int n, double t)
{
    ...
    double dEzdx, dEzdy, dEzdy_update;
    complex128 *Mx, *My;
    ...
    for(int j = 0; j < _J; j++) {
        for(int k = 0; k < _K; k++) {
            ...
            // Cylindrical update: compute rho
            //double rho_jp1k = 1.0*(j+2)/_dy;
            //double rho_jk = 1.0*(j+1)/_dy;
            //double rho_jp1k = 1.0*(j+2)*_dy;
            //double rho_jk = 1.0*(j+1)*_dy;
            double rho_jp1k = 1.0*(_j0+j+1+0.5)*_dy;
            double rho_jk;
            rho_jk = 1.0*(_j0+j+0.5)*_dy;
            //if(j==0){
            //    //rho_jk = 0.001*rho_jp1k;
            //    rho_jk = 0.001*rho_jp1k;
            //}
            //else{
            //    rho_jk = 1.0*(_j0+j)*_dy;
            //}
            
            // Update Hx
            dEzdy = ody*(_Ez[ind_jp1k] - _Ez[ind_jk]);
            dEzdy_update = ody*(_Ez[ind_jp1k] - _Ez[ind_jk]) + 0.5*_R*(_Ez[ind_jp1k]/rho_jp1k + _Ez[ind_jk]/rho_jk);
            //dEzdy_update = ody*(_Ez[ind_jp1k] - _Ez[ind_jk]);
            //dEzdy = ody*(_Ez[ind_jp1k] - _Ez[ind_jk]) + _R*(_Ez[ind_jk]/rho_jk);
            //dEzdy = _Ez[ind_jp1k]*(ody + 1.0/(2.0*rho_jp1k)) - _Ez[ind_jk]*(ody - 1.0/(2.0*rho_jk));
