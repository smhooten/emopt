...
h_lowerRidge = 0.5
wg_spacing = 2.0
angle_wg1 = 40.0 # degrees from x axis
angle_wg3 = 60.0 # degrees from x axis

h_triangle = 10.0
angle_triangle = 120.0 # degrees, 2 times angle from x axis

n_InP = 3.4
n_SiO2 = 1.444

ds = 0.02 # hyperbola line discretization in micron
input_x = w_pml + 3*dx
...
a1 = np.tan(angle_wg1*pi/180)*2/wg_spacing
a3 = np.tan(angle_wg3*pi/180)*2/wg_spacing

xx = np.linspace(0, converging_pt, ceil(converging_pt / ds))

yy1 = 2 * wg_spacing * (1 / (1 + np.exp(a1 * (xx - converging_pt))) - 0.5)
yy3 = 2 * wg_spacing * (1 / (1 + np.exp(a3 * (xx - converging_pt))) - 0.5)

index = np.argmin(np.abs(xx-input_x-3*dx))

diff1 = wg_spacing - yy1[index]
diff3 = wg_spacing - yy3[index]
