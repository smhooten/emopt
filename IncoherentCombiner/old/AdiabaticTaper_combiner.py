...
# taper
x_taper = np.array([0, X, X, 2*X/5, w_pml+1000, 0])
#y_taper = np.array([Y/2-w_wg/2, Y/2-w_wg/2, Y/2+w_wg/2, Y/2+w_wg/2,
#                    Y/2+w_wg_out/2, Y/2+w_wg_out/2, Y/2-w_wg_out/2, Y/2-w_wg_out/2])

y_taper= np.array([Y/2-wg_spacing/2-w_wg, Y/2-wg_spacing/2-w_wg,
                   Y/2-wg_spacing/2-w_wg+w_wg_out,
                   Y/2-wg_spacing/2-w_wg+w_wg_out, Y/2-wg_spacing/2,
                   Y/2-wg_spacing/2])
#^slanted taper
...
taper_spacer.layer = 2; taper_spacer.material_value = n_SiO2**2

taper_waveguide = emopt.grid.Polygon()
taper_waveguide.set_points(x_taper, y_taper)
taper_waveguide.layer = 2; taper_waveguide.material_value = n_InP**2

taper_lower = emopt.grid.Polygon()
taper_lower.set_points(x_taper, y_taper)
taper_lower.layer = 2; taper_lower.material_value = n_SiO2**2



x_input2 = np.array([0, 3*X/5, X-w_pml-1000, 2*X/5, w_pml+1000, 0])
y_input2 = np.array([Y/2+wg_spacing/2+w_wg, Y/2+wg_spacing/2+w_wg,
                     Y/2-wg_spacing/2-w_wg+w_wg_out,
                     Y/2-wg_spacing/2-w_wg+w_wg_out, Y/2+wg_spacing/2,
                     Y/2+wg_spacing/2])

in2_spacer = emopt.grid.Polygon()
in2_spacer.set_points(x_input2, y_input2)
in2_spacer.layer = 2; in2_spacer.material_value = n_SiO2**2

in2_waveguide = emopt.grid.Polygon()
in2_waveguide.set_points(x_input2, y_input2)
in2_waveguide.layer = 2; in2_waveguide.material_value = n_InP**2

in2_lower = emopt.grid.Polygon()
in2_lower.set_points(x_input2, y_input2)
in2_lower.layer = 2; in2_lower.material_value = n_SiO2**2


x_hole = np.array([w_pml+1000+3*w_wg, 3*X/5-2*w_wg, X-w_pml-1000-2*w_wg,
                   2*X/5+2*w_wg])
y_hole = np.array([Y/2+wg_spacing/2, Y/2+wg_spacing/2,
                   Y/2-wg_spacing/2-w_wg+w_wg_out,
                   Y/2-wg_spacing/2-w_wg+w_wg_out+200])

hole= emopt.grid.Polygon()
hole.set_points(x_hole, y_hole)
...
eps.add_primitive(in2_spacer, Z/2+h_waveguide/2, Z/2+h_waveguide/2+h_spacer)
eps.add_primitive(in2_waveguide, Z/2-h_waveguide/2, Z/2+h_waveguide/2)
eps.add_primitive(in2_lower, Z/2-h_waveguide/2-h_lowerRidge, Z/2-h_waveguide/2)
