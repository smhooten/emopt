...
def findRelativePhase(A,B)
    ...
    for x in np.nditer(A)
        phaseA.append(np.angle(x))

    for x in np.nditer(B)
        phaseB.append(np.angle(x))

    phase = np.add(phaseA, -phaseB)
...
eps.addprimitive(bg_substrate, 0, h_substrate)
eps.addprimitive(bg_air, h_substrate, Z)
...
sim1.set_sources([JxFund, JyFund, JzFund, MxFund, MyFund, MzFund], input_slice)
sim2.set_sources([JxHigher, JyHigher, JzHigher, MxHigher, MyHigher, MzHigher], input_slice)
...
if(NOT_PARALLEL):
    ...
    ax1.imshow(np.real(Ey), extent = [0,X-2*w_pml,Y-2*w_pml], vmin=-vmax,
...
if(NOT_PARALLEL):
    ...
    ax2.imshow(np.real(Ey), extent = [0,X-2*w_pml,Y-2*w_pml], vmin=-vmax,
              vmax=vmax, cmap='seismic')
    plt.show()
